...
| digest            | v.散播,传播,漫射,扩散,(使)慢慢混合;adj.散开的,弥漫的                                                                                |
| diffuse           | n.分类,摘要;vi.消化;vt.消化,融会贯通                                                                                         |
| dignity           | n.尊严,高贵                                                                                                          |
...
| notorious         | adj.声名狼籍的                                                                                                        |
| notwithstanding   | prep.虽然,尽管;adv.尽管,还是;conj.虽然,尽管                                                                                  |
...
| straightforward   | adj.正直的,坦率的,简单的,易懂的,直接了当的;adv.坦率地                                                                                |
...
| telecommunication | n.电讯,长途通讯,无线电通讯,电信学                                                                                              |
...
<script data-pjax>
  ...
  // 给你的房间起个专属代号，方便区分后续的其他房间
  const roomName = 'cet6_words_ScrollPos';

  // 2. 核心功能：读取记忆并执行精准空降
  function jumpToMemory() {
    let savedPos = localStorage.getItem(roomName);
  ...
  function recordMemory() {
    // 绝密条件：只有在未进入“卸载状态”时，才允许写入！
    // 这样就能完美避开 PJAX 卸载页面前那次致命的自动回零动作！
    if (!window.isUnloading) {
      localStorage.setItem(roomName, window.scrollY);
    }
  }

  // 4. 终极防御矩阵：监听页面的“死亡瞬间”
  function lockMemory() {
    window.isUnloading = true; // 页面即将消失，瞬间锁死！拒绝任何新的写入！
  }

  // 挂载各种离开页面的监听事件
  // 兼容普通刷新、关闭窗口、以及浏览器的前进后退按钮
  window.addEventListener('beforeunload', lockMemory);
  window.addEventListener('pagehide', lockMemory);
  // 特别针对 PJAX 的切页事件加固防线
  document.addEventListener('pjax:send', lockMemory); 

  // 5. 部署记忆引擎与防堆叠处理
  window.removeEventListener('scroll', recordMemory);
  window.addEventListener('scroll', recordMemory);
